...
import {Dialog,DialogBst, Loading} from './components'
...
function ActionBox(props) {
    ...
    const bodyComponent = function () {
        if (props.type == ActionBoxTypes.DIALOG_CONFIRM || props.type == ActionBoxTypes.DIALOG|| props.type == ActionBoxTypes.DIALOG_BST) {
            if(props.type == ActionBoxTypes.DIALOG_BST){
                return <DialogBst {...restProps}>{children}</DialogBst>
            }
            return <Dialog
                {...restProps}
            >
